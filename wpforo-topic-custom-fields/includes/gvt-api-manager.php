...
if (!class_exists('GVT_API_Manager')) {
    ...
    class GVT_API_Manager {
        ...
        public function __construct($plugin_filename, $plugin_option_slug, $admin_hook) {
            ...
            $this->plugin_slug = basename(dirname((string) $plugin_filename));
            $this->plugin_filename = basename((string) $plugin_filename);
        ...
        public function activation_form() {
            if (!$this->getSecret()) {
                ...
                if (!$this->getSecret()) {
                    ?>
                    <div class="gvt-activation-form <?php echo $this->plugin_slug;?>" style="border: 1px solid #ccc; padding: 10px; margin: 10px 0px 20px 0px; background: #dddddd;">
        ...
        public function activate_required() {
            ...
            if (!$this->getSecret() && $id_exist != 1 && !get_transient('dismiss_' . $this->plugin_slug)) {
                echo "<div class='gvt-activation-notice notice error  is-dismissible {$this->plugin_slug}'><p>Activate <a href='" . admin_url('admin.php?page=' . $this->plugin_option_slug) . "'> $this->plugin_name </a> plugin !</p>  $gvtUpdateMsg</div>";
            }
        }
